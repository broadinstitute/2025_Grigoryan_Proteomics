...
input_file <- "../00.exploratory_analysis/inputs/proteomics_data/normalized_signal.csv"
output_dist <- "./outputs/prot_distances.parquet"
...
all_dat$Metadata_Perturbation <- paste(all_dat$Metadata_Compound, all_dat$Metadata_compound_concentration, sep="_")
...
gmd_df <- data.frame()
for (plate in plates) {
  plate_dat <- all_dat[all_dat$Metadata_plate_barcode == plate, feat_cols]
  plate_dat <- as.matrix(plate_dat)
  plate_meta <- all_dat[all_dat$Metadata_plate_barcode == plate, meta_cols]
